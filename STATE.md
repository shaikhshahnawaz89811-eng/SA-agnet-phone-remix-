...
2. **[MEDIUM][OPEN] BUG-2 — Push branch hardcoded.** `git_ops.push()` me `branch="main"` fixed hai. Cloned repo ka default "master" ho to push fail hoga. Fix: push se pehle `git branch --show-current` se actual branch detect karo.
