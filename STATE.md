...
3. **[MEDIUM][OPEN] BUG-3 — MsgBox id collision risk.** File: `core/msgbox.py` Function: `add()`. `msgbox.add()` me id = `len(messages)+len(archived)+1` — persisted counter nahi hai. Archive ke baad do entries same id le sakti hai. Fix: `data_store` me persisted incrementing counter.
